//! Data-usage dialog, port of UI/UsageForm.cs: today's total on top, a SysListView32 of
//! per-day rx/tx/total newest-first, Reset (confirm) and Close.
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    IsDialogMessageW, MessageBoxW, MoveWindow, PostQuitMessage, SetForegroundWindow, SendMessageW,
    ShowWindow, TranslateMessage, HMENU, IDYES, MB_DEFBUTTON2, MB_ICONWARNING, MB_RIGHT, MB_RTLREADING,
    MB_YESNO, MSG, SW_SHOW, WINDOW_EX_STYLE, WM_CLOSE, WM_COMMAND, WM_CTLCOLORSTATIC, WM_DRAWITEM,
    WM_ERASEBKGND, WS_BORDER, WS_CAPTION, WS_CHILD, WS_EX_DLGMODALFRAME, WS_EX_LAYOUTRTL,
...
/// Modal data-usage dialog on the current thread (nested message loop). `usage` is a raw pointer
/// (not `&mut`) so the caller holds no borrow across the loop — the nested loop can re-enter the
/// app (tray click -> popup) which reads the same DataUsage; the pointer target outlives the call.
pub fn show(_owner: HWND, usage: *mut DataUsage) {
    unsafe {
    ...
    let title = window::utf16z(i18n::t("usage.title"));
...
fn w_class() -> PCWSTR {
    windows::core::w!("PrayerTrayUsage")
}
...
impl Dialog {
    fn usage(&mut self) -> &mut DataUsage {
        unsafe { &mut *self.usage }
    }
    ...
    fn build(&mut self) {
        // Today total label (styled via WM_CTLCOLORSTATIC + bold font).
        ...
        let cols = [
            (i18n::t("usage.date").to_string(), 110, LVCFMT_LEFT),
            (format!("↓ {}", i18n::t("usage.down")), 82, LVCFMT_RIGHT),
            (format!("↑ {}", i18n::t("usage.up")), 82, LVCFMT_RIGHT),
            (i18n::t("usage.total").to_string(), 90, LVCFMT_RIGHT),
        ];
        ...
        controls::button(self.hwnd, i18n::t("btn.resetUsage"), self.s(M), self.s(by), self.s(BTN_W), self.s(BTN_H), ID_RESET);
    ...
    fn refill(&mut self) {
        let (rx, tx) = self.usage().today();
        let today_text = i18n::f("usage.today", &[&data_usage::size(rx), &data_usage::size(tx)]);
        controls::set_text(self.item_today(), &today_text);

        unsafe {
            SendMessageW(self.list, LVM_DELETEALLITEMS, Some(WPARAM(0)), Some(LPARAM(0)));
        }
        let history = self.usage().history();
        for (row, (date, drx, dtx)) in history.iter().enumerate() {
            let r = row as i32;
            self.set_lv_text(r, 0, date);
            self.set_lv_text(r, 1, &data_usage::size(*drx));
            self.set_lv_text(r, 2, &data_usage::size(*dtx));
            self.set_lv_text(r, 3, &data_usage::size(drx + dtx));
        }
    }

    // The today label is the only STATIC child; find it by walking is overkill — it's built first.
    fn item_today(&self) -> HWND {
        // STATIC controls have no id here; grab the first child.
        unsafe {
    ...
    fn on_reset(&mut self) {
        let body = window::utf16z(i18n::t("usage.resetConfirm"));
        let caption = window::utf16z(i18n::t("usage.title"));
    ...
    fn finish(&mut self) {
        ...
        self.done = true;
        // Destroy (not just hide): this is a top-level window; a leaked one keeps receiving
        // broadcasts and would dispatch into the freed Dialog via stale GWLP_USERDATA.
        unsafe {
